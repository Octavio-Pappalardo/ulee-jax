...
def update_epoch(update_state, unused_, config):
    def _update_minbatch(train_state, batch_info):
        def _loss_fn(params, transitions, memories_batch, advantages, targets):
            ...
            # agent outputs
            pi, value, intrinsic_value = train_state.apply_fn(params, memories_batch, transitions.obs, memories_mask, method=ActorCriticTransformer.model_forward_train)
            log_prob = pi.log_prob(transitions.action)

            # extrinsic value loss
            extrinsic_value_pred_clipped = transitions.value + (value - transitions.value).clip(-config.clip_eps, config.clip_eps)
            ...
            intrinsic_value_pred_clipped = transitions.intrinsic_value + (intrinsic_value - transitions.intrinsic_value).clip(-config.clip_eps, config.clip_eps)
